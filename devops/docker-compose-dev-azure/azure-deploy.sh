...
resourcename="prueba"  # lower case, only
resourcegroup="prueba_rg"
...
# git clone https://github.com/apilogicserver/prueba.git
# cd prueba
...
az group create --name prueba_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group prueba_rg --sku S1 --is-linux
az container create --resource-group prueba_rg --name prueba --image apilogicserver/prueba:latest --dns-name-label prueba --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//prueba.westus.azurecontainer.io:5656'
